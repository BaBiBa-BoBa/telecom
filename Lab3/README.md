...
### Цель работы:
Приобретение знаний и практических навыков виспользовании программного обеспечения для моделирования
компьютерной сети.

### Материалы, оборудование, программное обеспечение:
Лаборатория, оснащенная персональными компьютерами, объединенными в локальную сеть
...
### Время самостоятельной подготовки:
2 часа.
...
Для моделирования сетей используют симуляторы (воспроизводят поведение, но не реальный код) и эмуляторы (запускают реальный код на виртуальном оборудовании). Примеры: Cisco Packet Tracer, GNS3, EVE-NG, NetSim, а также аналитические пакеты (MathWorks MATLAB/Simulink).
#### • Что такое IP-адрес?
IP-адрес — это уникальный числовой идентификатор (логический адрес) устройства в сети, работающей по протоколу IP. Он используется для маршрутизации пакетов и идентификации отправителя/получателя. Пример IPv4: 192.168.1.1.

#### • Что такое маска подсети?
Маска подсети — это 32-битное число (для IPv4), которое показывает, какая часть IP-адреса относится к адресу сети (префикс), а какая — к адресу узла (хоста). Пример: для адреса 192.168.1.10 с маской 255.255.255.0 — первые 3 байта (24 бита) — сеть, последний байт — узлы.

#### • Как работает концентратор?
Концентратор (hub) — это устройство физического уровня. Он работает так: получив сигнал на одном порту, он повторяет (регенерирует) его на всех остальных портах (широковещательная рассылка). Он не анализирует MAC-адреса и не разделяет коллизионные домены.

#### •Какие особенности обмена данными в ЛВС на основе концентраторов?

Основные особенности:
Один коллизионный домен — если два устройства передают одновременно, происходит коллизия (наложение сигналов), и данные искажаются.
Общая пропускная способность делится между всеми узлами.
Широковещательная доставка — каждый кадр видят все узлы сегмента (угроза безопасности).
Полудуплексный режим — устройство не может одновременно передавать и принимать.
Низкая эффективность при росте нагрузки из-за коллизий и необходимости алгоритма CSMA/CD.
...
3. Выполнить описанные ранее действия по ознакомлению с интерфейсом

### Задание 2. Непосредственное объединение в сеть двух компьютеров.
Рассмотреть простейший случай организации сети из двух компьютеров
без использования сетевого коммутационного оборудования.
### Выполнение задания:
1. Выполнить команду Файл-Новый, чтобы нарисовать схему сети как
показано на Рисункe 2.

![](Photo2.PNG)

Рис. 2. Схема компьютерной сети и ее модели в Netemul
...
Рис. 3. Cоздание соединений сетевых устройств
...
устройства, как показано на Рисунке 4, настроить сетевые интерфейсы
...
Рис. 4. Схема сети из двух ПК и концентратора
...
Рис. 5. Схема сети и ее модели на основе концентраторов
